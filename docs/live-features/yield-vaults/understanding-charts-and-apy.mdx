...
Every AI Hedge vault detail page provides real-time transparent metrics, historical performance charts, and underlying strategy breakdowns. This guide explains how to read each chart, how **Price Per Share (PPS)** measures real harvested yield, how **Instantaneous APY** is calculated, and the difference between **Forward-looking** and **Realized** strategy rates.
...
- $\text{Performance Fee}$ is deducted only from generated profits (e.g., 10%).
...
## 5. Metrics Comparison Cheat Sheet
...
## 6. Built on the ERC-4626 Vault Standard
...
- **Fair Fee Mechanics:** The 10% performance fee is assessed strictly upon successful `report()` harvest executions. If a strategy produces zero profit during a cycle, zero performance fee is taken.
